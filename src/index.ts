...
export class NoBinanceWalletError extends Error {
  public constructor() {
...
export class BinanceWallet extends Connector {
  ...
  public provider?: BinanceWalletProvider
  ...
  private async isomorphicInitialize(): Promise<void> {
    ...
    return (this.eagerConnection = import('./detectEthereumProvider').then(async (m) => {
      ...
      if (provider) {
        this.provider = provider as BinanceWalletProvider

        // handle the case when e.g. Binancewallet and metamask wallet are both installed
        if (this.provider.providers?.length) {
          this.provider = this.provider.providers.find((p) => p.isBinanceWallet) ?? this.provider.providers[0]
        }
