'use strict'
/**
 * Returns a Promise that resolves to the value of window.Binancewallet if it is
 * set within the given timeout, or null.
 * The Promise will not reject, but an error will be thrown if invalid options
 * are provided.
 *
 * @param options - Options bag.
 * @param options.museBeBinanceWallet - Whether to only look for BinanceWallet providers.
 * Default: false
 * @param options.silent - Whether to silence console errors. Does not affect
 * thrown errors. Default: false
 * @param options.timeout - Milliseconds to wait for 'ethereum#initialized' to
 * be dispatched. Default: 3000
 * @returns A Promise that resolves with the Provider if it is detected within
 * given timeout, otherwise null.
 */
function detectEthereumProvider({ museBeBinanceWallet = false, silent = false, timeout = 3000 } = {}) {
  _validateInputs()
  let handled = false
  return new Promise(resolve => {
    if (window.BinanceChain) {
      handleEthereum()
    } else {
      window.addEventListener('ethereum#initialized', handleEthereum, { once: true })
      setTimeout(() => {
        handleEthereum()
      }, timeout)
    }
    function handleEthereum() {
      if (handled) {
        return
      }
      handled = true
      window.removeEventListener('ethereum#initialized', handleEthereum)
      const { Binancewallet: BinanceChain } = window
      if (BinanceChain && (!museBeBinanceWallet || BinanceChain.isMetaMask)) {
        resolve(BinanceChain)
      } else {
        const message =
          museBeBinanceWallet && BinanceChain
            ? 'Non-BinanceWallet window.Binancewallet detected.'
            : 'Unable to detect window.Binancewallet.'
        !silent && console.error('@Binancewallet/detect-provider:', message)
        resolve(null)
      }
    }
  })
  function _validateInputs() {
    if (typeof museBeBinanceWallet !== 'boolean') {
      throw new Error(`@Binancewallet/detect-provider: Expected option 'museBeBinanceWallet' to be a boolean.`)
    }
    if (typeof silent !== 'boolean') {
      throw new Error(`@Binancewallet/detect-provider: Expected option 'silent' to be a boolean.`)
    }
    if (typeof timeout !== 'number') {
      throw new Error(`@Binancewallet/detect-provider: Expected option 'timeout' to be a number.`)
    }
  }
}
module.exports = detectEthereumProvider
//# sourceMappingURL=data:application/json;base64,
